...
def segment_color(image_path, lower_bound, upper_bound):
    ...
    display_image('Original Image', cv2.cvtColor(image, cv2.COLOR_BGR2HSV))
    display_image('Mask', mask, cmap='gray')
    display_image('Segmented Image', cv2.cvtColor(segmented_image, cv2.COLOR_BGR2RGB))


path = 'stop.png'
lb = [0, 200, 200]  # Lower bound of HSV for red color
ub = [5, 255, 255]  # Upper bound of HSV for red color
